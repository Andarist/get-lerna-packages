...
const getPackageConfigs = (lernaConfig, rootDirectory) => {
  if (lernaConfig.useWorkspaces) {
    const { workspaces } = readJSONFile(path.join(rootDirectory, PACKAGE_JSON))
    return workspaces.packages || workspaces
  }

  return lernaConfig.packages || [DEFAULT_PACKAGES_GLOB]
}
...
module.exports = (directory = process.cwd()) => {
  ...
  return getPackages(getPackageConfigs(lernaConfig, rootDirectory), rootDirectory)
}
